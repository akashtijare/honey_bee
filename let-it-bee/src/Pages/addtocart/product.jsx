export const product=[
    {
        "id":1,
        "image": "https://drive.google.com/uc?export=view&id=1RLonM1c5hYI3-K3r8Zr_9OXNFNPwCQeC",
        "name":"Bee Bread",
        "weight":"80 Gram",
        "description": "Bee bread is a small pellet containing pollen, along with nectar or honey. The exact bee bread composition varies depending on the plants the bees have foraged from. Often confused with bee pollen, bee bread is a fermented version, which is regarded as having a higher nutritional value. It is the bee’s saliva, containing a vast range of probiotic bacteria and yeast, which kicks off the fermentation process and increases the bioavailability of the super food.",
        "reviews":"f",
         cost:249,
        "category":"bars",
         quantity:1
    },
    {
        "id":2,
        "image":"https://drive.google.com/uc?export=view&id=1xgZGblXFw0CydohBWO6K_ZAASG7eixBh",
        "name":"Bee Propolis",
        "weight":"900 Grams",
        "description": "Bees produce a compound called propolis from the sap on needle-leaved trees or evergreens. It is used as a sealant for unwanted open spaces in the hive.",
        "reviews":"f",
         cost:2400,
        "category":"pollen",
        quantity:1
    },
    {
        "id":3,
        "image":"https://drive.google.com/uc?export=view&id=1g4szzB0e1QSTBtzqXvTj-j8dgLNqlt-s",
        "name":"Honey Bee Wax",
        "weight":"900 Grams",
        "description": "Natural Beeswax is Produced by the noble worker bee, beeswax is the substance you’ll find lining the structure of the honeycomb. Bees are vital to the health of our ecosystem, as they are responsible for sustaining one-third of the food we eat. An added bonus Beeswax has amazing skin health properties!",
        "reviews":"f",
        cost:550,
        "category":"bars",
        quantity:1
    },
    {
        "id":4,
        "image":"https://drive.google.com/uc?export=view&id=1hjBwAhPdrFU1vNA8KzCL39WkdVTt6BCO",
        "name":"Honey Comb",
        "weight":"1000 Grams",
        "description": "Absolute Natural Honeycomb, 100% Pure Unfiltered Honey Comb, 100% All-Natural, No Additives, No Preservatives, Fresh From The Farm, Honey in the comb",
        "reviews":"f",
        cost:240,
        "category":"honey",
        quantity:1
    },
    {
        "id":5,
        "image":"https://drive.google.com/uc?export=view&id=1iOiGbs3KWZamezF4P_D2fLr-BtLPFxWJ",
        "name":"Royal Jelly",
        "weight":"100 Grams",
        "description": "Royal Jelly is a natural bee product which is used for nutrition of young and adult queen bee. It has quiet complex nutritional profile containing unique organic acid, Polyphenols & nucleotides. It is great health supplement.",
        "reviews":"f",
         cost:1350,
        "category":"bars",
        quantity:1
    },
    {
        "id":6,
        "image":"https://drive.google.com/uc?export=view&id=1ItvH_hkMgZVMcGMe1j2nz_EcXqRPrC-q",
        "name":"Bee Pollen",
        "weight":"500 Grams",
        "description": "A very important activity of bees is pollination. During this process they collect pollen and pollinate the plant. Pollen is a very important item for survival and growth of bee colony. When it is in abundance we collect from the beehive without disturbing it. it is natural product loaded with nutrients, amino acids, vitamins, lipids and over 250 active substances. It can be digested easily and shall be taken without processing. Body builder will see lot of improvement. it is natural protein.",
        "reviews":"f",
        cost:400,
        "category":"pollen",
        quantity:1
    },
    {
        "id":7,
        "image":"https://drive.google.com/uc?export=view&id=1lJdXv0H8qSpS4o9aYEJHwouB2yZc50g0",
        "name":"Pollen Honey",
        "weight":"200 Grams",
        "description": "Pollen honey, also known as Pollen Infused honey, is considered to be a healthier alternative to Natural honey. Pollen honey contains antioxidants, vitamins, and minerals that are beneficial for overall health.",
        "reviews":"f",
         cost:750,
        "category":"honey",
        quantity:1
    },
    {
        "id":8,
        "image":"https://drive.google.com/uc?export=view&id=1xgZGblXFw0CydohBWO6K_ZAASG7eixBh",
        "name":"Bee Pollen Capsules",
        "weight":"750 mg",
        "description": "Bee Pollen Capsules are dietary supplements made from bee pollen, a powdery substance collected by bees from the stamen of plants. Bee pollen is a rich source of nutrients, including vitamins, minerals, amino acids, and antioxidants.",
        "reviews":"f",
         cost:490,
        "category":"pollen",
        quantity:1
    },
    {
        "id":9,
        "image":"https://drive.google.com/uc?export=view&id=1g4szzB0e1QSTBtzqXvTj-j8dgLNqlt-s",
        "name":"Bee Propolis Honey",
        "weight":"500 Grams",
        "description": "Propolis honey is a type of honey that is made by bees using the resin from trees and plants. Propolis honey has been shown to have strong antibacterial properties, making it useful for treating a variety of infections and wounds.",
        "reviews":"f",
         cost:900,
        "category":"honey",
         quantity:1
    },
    {
        "id":10,
        "image": "https://drive.google.com/uc?export=view&id=1RLonM1c5hYI3-K3r8Zr_9OXNFNPwCQeC",
        "name":"Bee Bread",
        "weight":"80 Gram",
        "description": "Bee bread is a small pellet containing pollen, along with nectar or honey. The exact bee bread composition varies depending on the plants the bees have foraged from. Often confused with bee pollen, bee bread is a fermented version, which is regarded as having a higher nutritional value. It is the bee’s saliva, containing a vast range of probiotic bacteria and yeast, which kicks off the fermentation process and increases the bioavailability of the super food.",
        "reviews":"f",
         cost:241,
        "category":"bars",
        quantity:1
    }
]